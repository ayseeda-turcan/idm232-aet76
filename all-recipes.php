...
<section class="search-results-page">
    <section class="intro">
        ...
        <p>Whether it's time for dinner, or you're craving a late night snack, our users are always coming up with new recipes for our community.</p>
    </section>
    ...
    <div class="search-header">
        ...
        <div class="filter-container">
            <form action="search.php" method="GET" id="filterForm">
                <?php if (!empty($search)): ?>
                    <input type="hidden" name="query" value="<?= htmlspecialchars($search, ENT_QUOTES); ?>">
                <?php endif; ?>

                <select name="category" onchange="document.getElementById('filterForm').submit()">
                    <option value="">All Categories</option>
                    <?php while($cat = $categories->fetch_assoc()): ?>
                        <option value="<?= htmlspecialchars($cat['category'], ENT_QUOTES); ?>" <?= ($category === $cat['category']) ? 'selected' : ''; ?>>
                            <?= htmlspecialchars($cat['category'], ENT_QUOTES); ?>
                        </option>
                    <?php endwhile; ?>
                </select>
                ...
                <select name="cook_time" onchange="document.getElementById('filterForm').submit()">
                    <option value="">Any Cook Time</option>
                    <option value="15" <?= ($max_cook_time === 15) ? 'selected' : ''; ?>>Under 15 min</option>
                    <option value="30" <?= ($max_cook_time === 30) ? 'selected' : ''; ?>>Under 30 min</option>
                    <option value="45" <?= ($max_cook_time === 45) ? 'selected' : ''; ?>>Under 45 min</option>
                    <option value="60" <?= ($max_cook_time === 60) ? 'selected' : ''; ?>>Under 1 hour</option>
                </select>

                <button><a href="search.php<?= !empty($search) ? '?query=' . urlencode($search) : ''; ?>">Clear Filters</a></button>
            </form>
        </div>
    </div>

    <?php if ($result && $result->num_rows > 0): ?>
        <p class="results-count"><?= $result->num_rows; ?> recipe(s) found</p>
